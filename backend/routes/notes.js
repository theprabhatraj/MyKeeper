...
router.post("/addnote", fetchuser, [body("title", "Enter a valid title").isLength({ min: 3 }), body("description", "Description  must be atleast six characters long").isLength({ min: 6 })], async (req, res) => {
  try {
...
router.delete("/deletenote/:id", fetchuser, async (req, res) => {
  try {
    ...
    //Allow deletetion only of the user owns this note
    if (note.user.toString() !== req.user.id) {
      return res.status(401).send("Not Allowed");
    }
    note = await Note.findByIdAndDelete(req.params.id);
    res.json({ Sucess: "The note has been successfully deleted", note: note });
  } catch (error) {
    console.error(error.message);
    res.status(500).send("some Internal server error occured");
  }
